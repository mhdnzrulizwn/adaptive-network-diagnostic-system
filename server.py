...
MANUAL_MAC_ADDRESS = None  # Set this to override automatic detection
...
class Layer2DiagnosticsServer:
    ...
    def process_agent_data_package(self, data_package, source_mac):
        ...
        try:
            ...
            message_type = data_package.get('message_type', '')
            ...
            if message_type == "layer2_ip_misconfigured":
                # Handle minimal Layer 2 IP misconfiguration alert
                reason = data_package.get('reason', 'Agent unable to reach server via HTTP/UDP due to IP misconfiguration.')
                logger.warning(f"Layer 2 IP misconfiguration alert from agent {agent_id} (MAC: {source_mac}): {reason}")

                # Log in events for dashboard flagging
                self.db_manager.log_event(agent_id, 'layer2_ip_misconfigured', 'warning', reason)

                # Optionally: Insert a minimal record in IP diagnostics for dashboard display
                self.db_manager.insert_ip_diagnostics(agent_id, {
                    'timestamp': data_package.get('timestamp', datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
                    'has_issues': True,
                    'issues': [reason],
                    'interfaces': [],
                    'connectivity': {'dns_resolution': False, 'ping_test': False, 'http_test': False},
                    'solutions': [],
                })

                logger.info(f"Flagged agent {agent_id} for IP misconfiguration via Layer 2 alert [METHOD: {COMM_METHOD_LAYER2}]")
                return  # Do not process as full diagnostics

            # ---- Existing logic for full diagnostics (if sent) ----
