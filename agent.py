...
SERVER_CONFIG = {
    ...
    'mac': 'your-server-mac-address',
    'interface_guid': 'your-server-int-guid',
    'name': 'Ethernet'
}

# Agent credentials
AGENT_CONFIG = {
    'interface_guid': '{your-agent-int-guid}',
    'mac_address': 'your-agent-mac-address',
    'name': 'Ethernet'
}
...
class Layer2Communicator:
    ...
    def __init__(self):
        ...
        self.interface = r"\\Device\\NPF_{your-agent-guid}"
        self.src_mac =  "your-agent-mac-address"
    ...
    def send_diagnostic_data(self, data: dict, server_mac: str = None) -> bool:
        ...
        try:
            ...
            payload = {
                    'message_type': 'layer2_ip_misconfigured',
                    'agent_id': data.get('agent_id'),
                    'hostname': socket.gethostname(),
                    'platform': platform.system(),
                    'agent_mac': self.src_mac,
                    'server_target': SERVER_CONFIG,
                    'reason': 'Unable to send diagnostics via HTTP/UDP due to IP misconfiguration.',
                    'timestamp': datetime.now().isoformat()
                }

            payload_bytes = json.dumps(payload).encode()
            frame = Ether(dst=dst_mac, src=self.src_mac, type=self.custom_ethertype) / Raw(load=payload_bytes)
            print("Layer2 Fallback Payload:", payload)
                ...
                if netifaces.AF_INET in addresses:
                    for addr_info in addresses[netifaces.AF_INET]:
                        ...
                        if ip and ip != '127.0.0.1':
                            ...
                            mac_nfi = addresses.get(netifaces.AF_LINK, [{}])[0].get('addr', '').lower()
                            is_up = False
                            # Find matching psutil interface by MAC
                            for ps_name, addrs in psutil_addrs.items():
                                for ps_addr in addrs:
                                    if ps_addr.family == psutil.AF_LINK and ps_addr.address.lower() == mac_nfi:
                                        is_up = psutil_stats[ps_name].isup
                            interfaces.append({
                                'interface': interface,
                                'ip': ip,
                                'netmask': netmask,
            ...
            if bytes_sent > 0:
                ...
                try:
                    ...
                    if ack_response.get('status') == 'received':
                        ...
                        return False  # Still consider it successful
                    ...
                    return False  # Still consider broadcast successful
